...
static int init(struct libmpv_pl_context *ctx, mpv_render_param *params)
{
    ...
    // gpu-next requires feature level 11 for its compute paths.
    ...
    // libplacebo retains the host device until pl_d3d11_destroy.
                    ...
                    pl_tex *out)
{
    ...
    // The caller destroys the wrapper before returning from render, releasing
    // libplacebo's COM reference. libplacebo reads dimensions from the texture.
    ...
    // Declared dimensions must match the wrapped texture.
